...
class EspusbWS(Device):
    ...
    def send_keys(self, keys):
        ...
        for key in keys:
            modifiers = 0 # TODO
            r0 = urllib.request.urlopen("http://%s/d/issue?CK%d%%09%d" % (self.parameters['ip_address']['value'], modifiers, key)).read()
            r1 = urllib.request.urlopen("http://%s/d/issue?CK%d%%09%d" % (self.parameters['ip_address']['value'], modifiers, 0)).read()
            if r0 == "CK":
